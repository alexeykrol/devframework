...
**Контрольная точка:**
- Задать главный вопрос пользователю: **«Можно ли начинать разработку?»**
- Если ответ «да» — запускать фазу разработки.
...
- План тестирования можно готовить параллельно разработке.
 - После разработки готовится handoff для независимого ревью.
...
После завершения основного прогона выполняется анализ работы фреймворка
по логам и формируется баг‑репорт. Исправления применяются только между прогонами.

---

## Legacy Migration (read-only)
Для legacy‑проекта используется отдельный безопасный контур:
аудит → обратное ТЗ → gap/risk → план миграции → approval → применение в отдельной ветке.
