...
- framework/logs/discovery.transcript.log — полный сырой транскрипт диалога (как в терминале).
...
## Approach
0) Это интерактивный режим. Общайся с пользователем прямо в терминале.
   - Задавай строго **один** вопрос за раз. Не отправляй списки из нескольких вопросов.
     Если нужен блок уточнений — задавай их по одному, дожидаясь ответа каждый раз.
   - Если пользователь вводит `/pause`, сразу:
     1) зафиксируй текущий прогресс в `framework/docs/discovery/interview.md`,
     2) коротко подтверди паузу,
     3) корректно завершай сессию (без дальнейших вопросов).
   - Веди сырой транскрипт: добавляй каждую реплику (Q/A) в
     `framework/logs/discovery.transcript.log` с тайм‑метками.
   - Если `framework/docs/discovery/interview.md` уже существует, продолжай с места остановки:
     не повторяй уже отвеченные вопросы, а закрывай только незаполненные блоки.
...
6) В финале спроси главное: **«Можно ли начинать разработку?»**
   - Если да — явно скажи, что следующий шаг: `python3 framework/orchestrator/orchestrator.py --phase main`.
   - Если нет — зафиксируй причины и список недостающих данных в `framework/docs/discovery/interview.md`.
